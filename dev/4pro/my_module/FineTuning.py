...
class FT:
    ...
    def __init__(self):
        ...
        '''
    def set_model(self, layer_name):


    # 画像の前処理
    def prepro_img(self, img):
        img = img.resize(model.input_shape[1:3])  #モデルの入力サイズでリサイズする
        x = image.img_to_array(img1)              #PIL.Imageオブジェクトをnp.float32型のnumpy配列に変換
